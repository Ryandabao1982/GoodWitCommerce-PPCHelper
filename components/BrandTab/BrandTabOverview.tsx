...
export const BrandTabOverview: React.FC<BrandTabOverviewProps> = ({
  brandState,
  ...
  const asins = Array.from(
    new Set(
      (brandState.campaigns || []).flatMap(
        (campaign) => campaign.asins || (campaign.asin ? [campaign.asin] : [])
      )
    )
  );
  ...
  const coverageData: Record<
    string,
    Record<string, { hasCoverage: boolean; hasOverlap: boolean }>
  > = {};
              ...
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  ...
                  {asins.map((asin) => (
                    <th
                      key={asin}
                      className="text-center p-2 font-semibold text-gray-700 dark:text-gray-300"
                    >
                      {asin}
                    </th>
                  ))}
                        ...
                        <td key={asin} className="p-2 text-center">
                          <div
                            className={`inline-block w-8 h-8 rounded ${
                              !cell.hasCoverage
                                ? 'bg-red-100 dark:bg-red-900 border-2 border-red-300 dark:border-red-700 cursor-pointer hover:bg-red-200 dark:hover:bg-red-800'
                                : cell.hasOverlap
                                  ? 'bg-yellow-100 dark:bg-yellow-900 border-2 border-yellow-300 dark:border-yellow-700 cursor-pointer hover:bg-yellow-200 dark:hover:bg-yellow-800'
                                  : 'bg-green-100 dark:bg-green-900 border-2 border-green-300 dark:border-green-700'
                            }`}
                            title={
                              !cell.hasCoverage
                                ? 'Click to create shell campaign'
                                : cell.hasOverlap
                                  ? 'Click to apply negative keyword'
                                  : 'Covered'
                            }
                          >
                            {!cell.hasCoverage && <span className="text-xs">+</span>}
                            {cell.hasOverlap && <span className="text-xs">⚠️</span>}
                            {cell.hasCoverage && !cell.hasOverlap && (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-700">
            ...
            <div className="text-xs text-blue-700 dark:text-blue-300">
              ...
              <br />• Negate: 8 keywords ready (≥15 clicks, 0 sales)
            </div>
          </div>
        </div>
      ) : (
