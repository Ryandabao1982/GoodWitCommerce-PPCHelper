...
export const BrandTabCampaigns: React.FC<BrandTabCampaignsProps> = ({ brandState, portfolios }) => {
  ...
  const handleCreateShell = (type: string) => {
    alert(
      `Creating ${type} campaign template. This will create a new campaign structure optimized for this strategy.`
    );
  };
  ...
  const suggestions = useMemo(() => {
    ...
    const noBudgetCampaigns = brandState.campaigns.filter(
      (c) => !c.dailyBudget || c.dailyBudget === 0
    );
    ...
    portfolioStats.forEach((p) => {
      if (p.budgetUtilization < 50 && p.campaignCount > 0) {
        items.push(
          `${p.name} portfolio is underutilized (${p.budgetUtilization.toFixed(0)}% budget used)`
        );
      }
    });
    ...
    if (!hasExactMatch)
      items.push('Consider creating an Exact Match campaign for high-performing keywords');
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full transition-all ${
                        portfolio.budgetUtilization >= 80
                          ? 'bg-green-500'
                          : portfolio.budgetUtilization >= 50
                            ? 'bg-yellow-500'
                            : 'bg-red-500'
                      }`}
                            ...
                            >
                              ...
                              {adGroup.defaultBid && (
                                <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                  Bid: ${adGroup.defaultBid.toFixed(2)} •{' '}
