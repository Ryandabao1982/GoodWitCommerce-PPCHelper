...
import { isViewAccessible } from './utils/navigation';
...
const App: React.FC = () => {
  ...
  const hasActiveBrand = Boolean(activeBrand);
  const hasKeywords = allBrandKeywords.length > 0;

  const handleViewChange = useCallback(
    (view: ViewType) => {
      const context = { hasActiveBrand, hasKeywords };

      if (!isViewAccessible(view, context)) {
        if (!hasActiveBrand) {
    ...
    [hasActiveBrand, hasKeywords, setCurrentView, setIsBrandModalOpen]
  );
