...
const App: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const normalizedStates = Object.fromEntries(
          Object.entries(loadedBrandStates).map(([name, state]) => [
            name,
            normalizeBrandState(state),
          ])
        );
        setBrandStates(normalizedStates);
      } catch (error) {
        ...
        setBrandStates(normalizedFallback);
      }
    };
    loadData();
  }, []);
  ...
  const updateBrandState = useCallback((brandName: string, updates: Partial<BrandState>) => {
    setBrandStates((prev) => {
      const existingState = prev[brandName]
        ? normalizeBrandState(prev[brandName])
        : normalizeBrandState({
            keywordResults: [],
            searchedKeywords: [],
      ...
      const nextCampaigns = updates.campaigns
        ...
        : existingState.campaigns;

      const nextState = normalizeBrandState({
        ...existingState,
        ...updates,
        campaigns: nextCampaigns,
        negativeKeywordLists:
          updates.negativeKeywordLists ?? existingState.negativeKeywordLists ?? [],
      });
      ...
      brandStateStorage.update(brandName, nextState).catch((error) => {
